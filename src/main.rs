...
// THE BINARY (NG+ roster, debuts w11). Two rocks bound together, turning about their shared centre.
// The rocks are ordinary; the LINK between them is the hazard — a sweeping lethal bar you have to time
// rather than an object you dodge. It's the first rock in the game that's about SPACE instead of about
// itself, and the counter is honest: break either end and the bond dies with it (the survivor is flung
// off along its tangent, so killing one is a real answer rather than a half-measure).
const BINARY_LINK: f32 = 96.0; // px between the pair — wide enough to be a lane you must time
const BINARY_SPIN: f32 = 1.15; // rad/s about the shared centre
const BINARY_R: f32 = 7.0; // half-thickness of the lethal link
const BINARY_FLING: f32 = 165.0; // px/s the survivor keeps when the bond breaks (its tangential speed)
const _: () = assert!(BINARY_FLING < MAX_SPEED); // SPEED LAW: a freed half must never outrun the ship
// the pair must not be so tight that the link is hidden inside the two rocks it joins — then it's an
// invisible hazard, which is just unfair
const _: () = assert!(BINARY_LINK > 60.0);

// THE SUNDER (NG+ roster, debuts w13). Inert until it dies, and then it SHOVES: a shock ring that
// throws every loose rock (and the ship) outward. No damage of its own — the danger is entirely what
// it puts in your path, so your own kills become positioning decisions. It is the counter-pressure to
// a calm field: chaos generated by YOUR actions rather than by more spawns.
const SUNDER_PUSH_R: f32 = 300.0; // reach of the shove
const SUNDER_PUSH: f32 = 620.0; // px/s of velocity handed to something at the centre (falls off with distance)
const SUNDER_SHIP_MULT: f32 = 0.55; // the ship is shoved less than the rocks — it's a nudge, not a launch
// The shove is an IMPULSE, so what matters for the SPEED LAW is the speed a rock ends up at, not the
// push itself: 620 added to a rock already drifting at 150 leaves it outrunning the ship, which is
// exactly what the law forbids. Shoved rocks are therefore CLAMPED, not just pushed gently.
const SUNDER_ROCK_CAP: f32 = 390.0; // dramatic, and still comfortably outrunnable
const _: () = assert!(SUNDER_ROCK_CAP < MAX_SPEED); // ← the check the ship-only assert below missed
const _: () = assert!(SUNDER_PUSH * SUNDER_SHIP_MULT < MAX_SPEED); // you can always fly out of your own mess
...
// The Binary: a cold bound-iron blue-white, so the LINK reads brightly against the field.
fn binary_color() -> Color {
    Color::srgb(0.55, 0.95, 1.25)
}
// The Sunder: a heavy amber-bronze — it looks like something under pressure.
fn sunder_color() -> Color {
    Color::srgb(1.25, 0.72, 0.24)
}
...
// One half of a BINARY pair. `other` is its partner and `phase` its angle about the shared centre;
// only the half holding `lead` drives the pair's motion, so the two can never fight each other for it.
...
// A SUNDER rock: harmless until broken, then it shoves the neighbourhood.
#[derive(Component)]
struct Sunder;
...
const ACH_BONDS: u32 = 250; // one per PAIR, not per rock — so this is 250 whole binaries answered
const ACH_SUNDERS: u32 = 300;
...
fn ach_meta(a: Ach) -> (&'static str, &'static str) {
    match a {
        ...
        Ach::IrreconcilableDifferences => ("Irreconcilable Differences", "Break 250 binary bonds and send both halves their separate ways"),
        Ach::YouStartedIt => ("You Started It", "Set off 300 sunders — every rock they threw at you was your own doing"),
...
fn ach_met(a: Ach, s: &Stats) -> bool {
    match a {
        ...
        Ach::IrreconcilableDifferences => s.bonds >= ACH_BONDS,
        Ach::YouStartedIt => s.sunders >= ACH_SUNDERS,
...
fn ach_progress(a: Ach, s: &Stats) -> Option<(u32, u32)> {
    match a {
        ...
        Ach::IrreconcilableDifferences => Some((s.bonds, ACH_BONDS)),
        Ach::YouStartedIt => Some((s.sunders, ACH_SUNDERS)),
...
struct Stats {
    ...
    // Counted where the EVENT happens rather than through `credit_rock_kill`: a snapped bond and a
    // triggered shove are the interesting actions, and neither is really "a rock died".
    bonds: u32,        // BINARY bonds broken (lifetime)
    sunders: u32,      // SUNDER shoves you set off (lifetime)
...
enum RockKind {
    ...
    Binary,  // NG+ roster — a bound pair; the LINK between them is the hazard
    Sunder,  // NG+ roster — inert until killed, then it shoves everything outward
...
fn spawn_kind_rock(commands: &mut Commands, pos: Vec2, size: u8, vel: Vec2, rng: &mut impl Rng, kind: RockKind) -> Entity {
    ...
    match kind {
        ...
        RockKind::Sunder => {
            commands.entity(e).insert(Sunder);
        }
        RockKind::Binary => {
            // ONE call, TWO rocks: a binary is meaningless alone, so the partner is spawned here and
            // the pair is cross-linked immediately. They share a velocity, so the whole assembly
            // drifts as one body while spinning about its centre (see `binary_update`).
            let phase = rng.gen_range(0.0..TAU);
            let off = Vec2::from_angle(phase) * (BINARY_LINK * 0.5);
            let mate = spawn_asteroid(commands, pos - off, size, vel, rng, false);
            commands.entity(e).insert(Binary { other: mate, phase, lead: true });
            commands.entity(mate).insert(Binary { other: e, phase: phase + std::f32::consts::PI, lead: false });
            // nudge the LEAD half onto its own side of the shared centre
            commands.entity(e).insert(Transform::from_xyz(pos.x + off.x, pos.y + off.y, 0.0));
        }
        RockKind::Red => {
...
fn roll_ngplus_kind(level: i32, rng: &mut impl Rng) -> RockKind {
    ...
    // carry waves 6-7, the Facet debuts at 8 (wave 8 is mostly mirrors — its teaching wave), the Husk
    // at 9, the Binary at 11 and the Sunder at 13. Each debut wave is WEIGHTED HEAVILY toward the new
    // type and then settles to a background rate, so lap two keeps teaching instead of just escalating.
    // The roster rule means everything past wave 5 comes from here; widen as new types land.
    let cw = content_wave(level);
    // Sunder (shoves on death) — rolled first so nothing eats its slice on its own teaching wave
    if cw >= 13 && rng.gen_bool(if cw == 13 { 0.5 } else { 0.24 }) {
        return RockKind::Sunder;
    }
    // Binary (a bound pair — ONE roll spawns TWO rocks, so its rate is deliberately the lowest of
    // the roster: at parity with the others it would own the whole field's population budget)
    if cw >= 11 && rng.gen_bool(if cw == 11 { 0.3 } else { 0.14 }) {
        return RockKind::Binary;
    }
...
// THE BINARY turns about its shared centre, and its LINK is lethal. Only the `lead` half drives the
// motion so the two can't fight over it; the follower is placed opposite, every frame, from the same
// centre — which is what keeps the bar rigid instead of letting collisions stretch it.
//
// The bond is FRAGILE BY DESIGN: the moment either half dies the other is freed, flung along its
// tangent at the speed it was already travelling. Killing one end is a real answer to the hazard
// rather than half a job, and it converts a timing problem into an ordinary drifting rock.
#[allow(clippy::too_many_arguments)]
fn binary_update(
    time: Res<Time>,
    mut commands: Commands,
    ...
    mut sfx: EventWriter<SoundFx>,
    dev: Res<Dev>,
    ships: Query<(Entity, &Transform, &Ship), Without<Binary>>,
    mut pairs: Query<(Entity, &mut Transform, &mut Velocity, &mut Binary)>,
) {
    let dt = time.delta_secs();
    // snapshot first: the follower's placement needs the lead's post-move state, and we can't hold two
    // mutable handles into the same query at once
    let snap: Vec<(Entity, Entity, Vec2, Vec2, f32, bool)> = pairs
        .iter()
        .map(|(e, t, v, b)| (e, b.other, t.translation.truncate(), v.0, b.phase, b.lead))
        .collect();
    for (e, other, pos, vel, phase, lead) in snap {
        // partner gone → the bond snaps and this half flies free along its tangent
        if pairs.get(other).is_err() {
            if let Ok((_, _, mut v, _)) = pairs.get_mut(e) {
                let tangent = Vec2::from_angle(phase + std::f32::consts::FRAC_PI_2);
                v.0 = vel + tangent * BINARY_FLING;
            }
            commands.entity(e).remove::<Binary>();
            // one bond, one tally: only the half that is still standing counts it, so a pair whose
            // two halves die in the same blast can't score twice
            if let Some(s) = stats.as_mut() {
                s.bonds += 1;
            }
            continue;
        ...
        if !lead {
            continue; // the lead half places this one
        }
        let new_phase = phase + BINARY_SPIN * dt;
        let arm = Vec2::from_angle(new_phase) * (BINARY_LINK * 0.5);
        let centre = pos - Vec2::from_angle(phase) * (BINARY_LINK * 0.5);
        let centre = centre + vel * dt; // the assembly drifts as one body
        if let Ok((_, mut t, _, mut b)) = pairs.get_mut(e) {
            b.phase = new_phase;
            t.translation = Vec3::new(centre.x + arm.x, centre.y + arm.y, 0.0);
        }
        if let Ok((_, mut t, mut v, mut b)) = pairs.get_mut(other) {
            b.phase = new_phase + std::f32::consts::PI;
            t.translation = Vec3::new(centre.x - arm.x, centre.y - arm.y, 0.0);
            v.0 = vel; // both halves carry the assembly's velocity, so a freed half keeps drifting sanely
        }
        // ── THE LINK IS THE HAZARD ── the two rocks kill on contact like any other rock (that's the
        // normal collision path); what's new is the BAR between them. Crossing it is fatal, so the
        // pair sweeps a lane you have to time rather than an obstacle you steer around. Checked from
        // the lead half only, once per pair.
        if run.respawn <= 0.0 {
            let (a, b2) = (centre + arm, centre - arm);
            for (se, st, sh) in &ships {
                if immune(sh, &dev) {
                    continue;
                }
                let sp = st.translation.truncate();
                let rr = BINARY_R + SHIP_R;
                if seg_dist2(sp, a, b2) < rr * rr {
                    kill_ship(&mut commands, &mut run, &mut next, &mut sfx, se, sp, &mut rand::thread_rng());
                }
            }
        }
    }
...
// THE SUNDER shoves when it dies. Detected via `RemovedComponents` rather than by threading another
// flag through `Flavor` and a dozen call sites: the rock can be killed by a bullet, the beam, a blast,
// the warp or a boss, and this catches every one of those paths without touching any of them.
fn sunder_update(
    mut commands: Commands,
    mut stats: Option<ResMut<Stats>>, // Option so headless tests needn't insert it
    mut removed: RemovedComponents<Sunder>,
    mut last: Local<HashMap<Entity, Vec2>>,
    arena: Res<Arena>,
    mut sfx: EventWriter<SoundFx>,
    sunders: Query<(Entity, &Transform), With<Sunder>>,
    mut rocks: Query<(&Transform, &mut Velocity), (With<Asteroid>, Without<Ship>, Without<Shielded>)>,
    mut ships: Query<(&Transform, &mut Velocity), With<Ship>>,
) {
    for (e, t) in &sunders {
        last.insert(e, t.translation.truncate());
    }
    let mut rng = rand::thread_rng();
    for e in removed.read() {
        let Some(p) = last.remove(&e) else { continue };
        // a sunder CULLED off-screen doesn't get to shove the field from out there
        if p.x.abs() > arena.half.x || p.y.abs() > arena.half.y {
            continue;
        }
        let sc = sunder_color();
        burst(&mut commands, p, sc, 34, 420.0, &mut rng);
        commands.spawn((
            Shockwave { age: 0.0, ttl: 0.34, max_r: SUNDER_PUSH_R, color: sc },
            Transform::from_xyz(p.x, p.y, 0.0),
        ));
        sfx.write(SoundFx::Mine); // a heavy thump — it is a physical shove, not an explosion
        if let Some(s) = stats.as_mut() {
            s.sunders += 1;
        }
        // the shove itself: falls off with distance, and NOTHING takes damage from it
        let push = |q: Vec2, v: &mut Vec2, mult: f32| {
            let to = q - p;
            let d = to.length();
            if d < SUNDER_PUSH_R && d > 0.01 {
                *v += to / d * SUNDER_PUSH * (1.0 - d / SUNDER_PUSH_R) * mult;
            }
        };
        for (t, mut v) in &mut rocks {
            let q = t.translation.truncate();
            push(q, &mut v.0, 1.0);
            // clamp AFTER the impulse: the rock's own drift is already in there
            if v.0.length() > SUNDER_ROCK_CAP {
                v.0 = v.0.normalize() * SUNDER_ROCK_CAP;
            }
        }
        for (t, mut v) in &mut ships {
            let q = t.translation.truncate();
            push(q, &mut v.0, SUNDER_SHIP_MULT); // you get shoved too — less hard, and never harmed
        }
    }
}
...
fn slinger_update(
    ...
    for (se, mut st, mut sl) in &mut slingers {
        ...
        if sl.ammo.is_none() {
            // COOL, then TRACTOR-GRAB a free field rock (green on wave 15 → hard to shoot away).
            //
            // ⭐ NG+ CLOSE HAUL: on lap two it takes the rock nearest to YOU rather than the one nearest
            // to itself. The rock you're sheltering behind becomes the round fired at you — a direct
            // answer to cover, which is otherwise a reliable way to sit out its barrage (mob fire is
            // stopped by rocks, so hugging one is a real tactic). It also drags the fight to wherever
            // you are instead of letting you fight it at the range you choose.
            sl.cool -= dt;
            if sl.cool <= 0.0 {
                let from = if plus.0 { ship_pos.unwrap_or(p) } else { p };
                if let Some((re, _)) = grabbable.iter().min_by(|(_, a), (_, b)| {
                    let (da, db) = (a.translation.truncate().distance_squared(from), b.translation.truncate().distance_squared(from));
                    da.total_cmp(&db)
                }) {
...
fn render_extras(
    ...
    // the link needs BOTH halves' positions, so the pair is read through two handles into the same set
    binaries: Query<(&Binary, &Transform)>,
    binary_others: Query<&Transform, With<Binary>>,
    ...
    // ── BINARY LINKS ── drawn AFTER the rocks so the bar reads on top of both halves. The bar is the
    // hazard, so it's the brightest thing about the pair: a solid core with a hot centre, plus short
    // cross-ties along it so it reads as a rigid BOND rather than a laser. Only the lead half draws,
    // so the line isn't painted twice.
    for (b, bt) in &binaries {
        if !b.lead {
            continue;
        }
        let Ok(ot) = binary_others.get(b.other) else { continue };
        let (a, z) = (bt.translation.truncate(), ot.translation.truncate());
        let bc = binary_color();
        let perp = (z - a).normalize_or_zero().perp();
        gizmos.line_2d(a, z, bc);
        gizmos.line_2d(a + perp * 2.0, z + perp * 2.0, dim(bc, 0.5));
        gizmos.line_2d(a - perp * 2.0, z - perp * 2.0, dim(bc, 0.5));
        // rungs: a few short cross-ties, which is what makes it read as a bar under tension
        let n = 5;
        for k in 1..n {
            let q = a + (z - a) * (k as f32 / n as f32);
            gizmos.line_2d(q + perp * 5.0, q - perp * 5.0, dim(bc, 0.75));
        }
        // the shared centre it turns about — a small bright pip, so the rotation is legible
        gizmos.circle_2d(Isometry2d::from_translation((a + z) * 0.5), 2.2, dim(bc, 1.2));
    }
...
fn gallery_bit(art: GalleryArt) -> u32 {
    let i = match art {
        ...
        GalleryArt::Rock(RockKind::Binary) => 22,
        GalleryArt::Rock(RockKind::Sunder) => 23,
...
fn gallery_entries(s: &Stats) -> Vec<(GalleryArt, &'static str, &'static str, &'static str, bool)> {
    vec![
        ...
        (GalleryArt::Rock(RockKind::Binary), "BINARY", "Something tied these together",
         "Two rocks turning about a point in the empty space between them, and the space between them is not empty — there is a BAR there, and it will cut my ship in half. The rocks themselves are nothing special. It is the bar that kills, and it sweeps, so the only way through is to wait for it to come round. Break either rock and the other goes tumbling off free, which is the answer, and it is also the thing I cannot stop thinking about: a bond that fails when you break one end is a bond somebody ENGINEERED. Rock does not do this.", gallery_seen(s, GalleryArt::Rock(RockKind::Binary))),
        (GalleryArt::Rock(RockKind::Sunder), "SUNDER", "It has been holding its breath",
         "It does nothing. It drifts, it does not chase, it does not shield, it does not hide — and then you break it and it lets go of everything it was holding, and half the field goes flying, me included. It does no damage. It does not have to. It just puts three rocks where I was about to be. I have learned to check what is behind one before I shoot it, which means the Belt has taught me to think about consequences, and I do not love what that implies about whoever put it here.", gallery_seen(s, GalleryArt::Rock(RockKind::Sunder))),
...
fn gallery_sightings(
    ...
    // the two newest rocks ride their own query — the tuple above is at Bevy's element limit
    fresh_rocks: Query<(Option<&Binary>, Option<&Sunder>), With<Asteroid>>,
    mines: Query<(), With<Mine>>,
    ...
    let mut fresh = false;
    for (binary, sunder) in &fresh_rocks {
        if binary.is_some() {
            fresh |= mark_seen(&mut stats, GalleryArt::Rock(RockKind::Binary));
        }
        if sunder.is_some() {
            fresh |= mark_seen(&mut stats, GalleryArt::Rock(RockKind::Sunder));
        }
    }
    ...
    match art {
        ...
        GalleryArt::Rock(kind) => {
            ...
            let col = match kind {
                ...
                RockKind::Binary => binary_color(),
                RockKind::Sunder => sunder_color(),
            ...
            match kind {
                ...
                RockKind::Binary => {
                    // the PAIR and the bar between them — the bar IS the entry, so it's drawn bright
                    let a = t * 0.5;
                    let arm = Vec2::from_angle(a) * r * 1.25;
                    for s in [-1.0f32, 1.0] {
                        gizmos.linestrip_2d(gallery_rock_ring(c + arm * s, r * 0.55), col);
                    }
                    gizmos.line_2d(c - arm, c + arm, col);
                    gizmos.line_2d(c - arm, c + arm, dim(Color::WHITE, 0.6));
                    gizmos.circle_2d(Isometry2d::from_translation(c), r * 0.1, dim(col, 0.7)); // the shared centre
                }
                RockKind::Sunder => {
                    // the rock, wrapped in the shove it's holding: expanding rings and outward arrows
                    for k in 1..=2 {
                        gizmos.linestrip_2d(gallery_rock_ring(c, r * (1.0 + 0.42 * k as f32)), dim(col, 0.34 / k as f32));
                    }
                    for i in 0..8 {
                        let d = Vec2::from_angle(i as f32 / 8.0 * TAU + t * 0.4);
                        gizmos.line_2d(c + d * r * 1.15, c + d * r * 1.75, dim(col, 0.7));
                    }
                }
                RockKind::Cluster => {
...
fn save_progress(s: &Stats) {
    let line = format!(
        ...
        s.facet,
        s.husk,
        // APPEND ONLY past this point. Inserting a field mid-list shifts every index after it, so an
        // existing save would read its husk count as its facet count and so on.
        s.bonds,
        s.sunders
    );
...
mod tests {
    ...
    fn the_nearest_grind_ticker_picks_the_closest_counter() {
        ...
        let done = Stats {
            ...
            bonds: ACH_BONDS, sunders: ACH_SUNDERS,
    ...
    fn finale_roll_covers_every_rock_type() {
        ...
        for _ in 0..4000 {
            match roll_finale_kind(&mut rng) {
                ...
                RockKind::Lapse | RockKind::Facet | RockKind::Husk | RockKind::Binary | RockKind::Sunder => {
                    panic!("the NG+ roster (Lapse/Facet/Husk/Binary/Sunder) must never appear in the BASE finale mix")
                }
            }
        }
        ...
        fn sample(level: i32, n: usize, rng: &mut rand::rngs::ThreadRng) -> (i32, i32, i32, i32) {
            ...
            for _ in 0..n {
                match roll_rock_kind(level, false, rng) {
                    ...
                    RockKind::Lapse | RockKind::Facet | RockKind::Husk | RockKind::Binary | RockKind::Sunder => {
                        panic!("the NG+ roster (Lapse/Facet/Husk/Binary/Sunder) must never appear in a BASE-game wave")
                    }
                }
            }
    ...
    fn hunters_debut_on_wave_six_and_retire_with_their_act() {
        ...
        for _ in 0..600 {
            match roll_rock_kind(12, true, &mut rng) {
                ...
                RockKind::Facet | RockKind::Husk | RockKind::Binary | RockKind::Sunder => {} // debut at 8/9/11/13; counted by their own tests
    ...
    fn mob_app(mob_at: Vec2, ship_at: Vec2, fleeing: bool) -> (App, Entity) {
        ...
        app.insert_resource(Stats::default());
        app.insert_resource(RunFlags::default());
        app.insert_resource(Arena { half: Vec2::new(1200.0, 800.0) }); // roomy — no edge bounce in the way
        app.insert_resource(Wave { level: 1, timer: WAVE_SECS, calm: 0.0 });
        // TimePlugin re-derives `Time` from the wall clock every update, so hand-advancing it gets
        ...
        app.world_mut().spawn((Ship { angle: 0.0, cooldown: 0.0, invuln: 0.0, flame: 0.0 }, Velocity(Vec2::ZERO), Transform::from_xyz(ship_at.x, ship_at.y, 0.0)));
        app.add_systems(Update, enemy_update);
        (app, mob)
    }
    ...
    // A BINARY is only a binary while BOTH halves live. The bond snapping is the player's reward for
    // killing one end, and it's the one behaviour that can't be seen by reading either half alone.
    #[test]
    fn breaking_one_half_of_a_binary_frees_the_other() {
        let mut app = App::new();
        app.add_plugins(MinimalPlugins);
        app.add_event::<SoundFx>();
        app.insert_resource(bevy::time::TimeUpdateStrategy::ManualDuration(std::time::Duration::from_millis(50)));
        ...
        app.insert_resource(Run { lives: 3, respawn: 0.0, ..default() });
        app.insert_resource(NextState::<GameState>::default());
        let a = app.world_mut().spawn((Velocity(Vec2::new(40.0, 0.0)), Transform::from_xyz(BINARY_LINK * 0.5, 0.0, 0.0))).id();
        let b = app.world_mut().spawn((Velocity(Vec2::new(40.0, 0.0)), Transform::from_xyz(-BINARY_LINK * 0.5, 0.0, 0.0))).id();
        app.world_mut().entity_mut(a).insert(Binary { other: b, phase: 0.0, lead: true });
        app.world_mut().entity_mut(b).insert(Binary { other: a, phase: std::f32::consts::PI, lead: false });
        app.add_systems(Update, binary_update);
        app.update();
        app.update();
        // while both live the pair holds its separation — that rigidity is what makes the bar a bar
        let sep = {
            let (pa, pb) = (app.world().entity(a).get::<Transform>().unwrap().translation, app.world().entity(b).get::<Transform>().unwrap().translation);
            (pa - pb).length()
        };
        assert!((sep - BINARY_LINK).abs() < 1.0, "the bond holds its length, got {sep}");
        assert!(app.world().entity(a).get::<Binary>().is_some(), "and both halves are still bound");
        // now kill one end
        app.world_mut().entity_mut(b).despawn();
        app.update();
        assert!(app.world().entity(a).get::<Binary>().is_none(), "the survivor is released — killing one end is a real answer");
        let v = app.world().entity(a).get::<Velocity>().unwrap().0;
        assert!(v.length() > 40.0, "and it's flung along its tangent, not left drifting as before ({v:?})");
        assert!(v.length() < MAX_SPEED, "SPEED LAW: a freed half can still be outrun");
        assert_eq!(app.world().resource::<Stats>().bonds, 1, "the bond counts once, from the surviving half only");
    }

    // The SUNDER's shove is an IMPULSE, so the Speed Law has to be enforced on the RESULT: a rock
    // already drifting fast would otherwise end up outrunning the ship, which the law forbids. The
    // ship-only assert next to the constants does NOT catch that case.
    #[test]
    fn a_sunder_shoves_hard_but_never_past_the_speed_law() {
        let mut app = App::new();
        app.add_plugins(MinimalPlugins);
        app.add_event::<SoundFx>();
        app.insert_resource(bevy::time::TimeUpdateStrategy::ManualDuration(std::time::Duration::from_millis(50)));
        app.insert_resource(Arena { half: Vec2::new(700.0, 400.0) });
        app.insert_resource(Stats::default());
        let sunder = app.world_mut().spawn((Sunder, Transform::from_xyz(0.0, 0.0, 0.0))).id();
        // a rock right beside it, ALREADY travelling fast — the case the naive assert misses
        let near = app.world_mut().spawn((
            Asteroid { size: 1, verts: vec![Vec2::X * 22.0], rot: 0.0, spin: 0.0, dense: false, hp: 1 },
            Velocity(Vec2::new(300.0, 0.0)),
            Transform::from_xyz(30.0, 0.0, 0.0),
        )).id();
        // and one comfortably outside the shove
        let far = app.world_mut().spawn((
            Asteroid { size: 1, verts: vec![Vec2::X * 22.0], rot: 0.0, spin: 0.0, dense: false, hp: 1 },
            Velocity(Vec2::ZERO),
            Transform::from_xyz(SUNDER_PUSH_R + 120.0, 0.0, 0.0),
        )).id();
        let ship = app.world_mut().spawn((Ship { angle: 0.0, cooldown: 0.0, invuln: 0.0, flame: 0.0 }, Velocity(Vec2::ZERO), Transform::from_xyz(0.0, 60.0, 0.0))).id();
        app.add_systems(Update, sunder_update);
        app.update(); // records its position
        app.world_mut().entity_mut(sunder).despawn();
        app.update(); // …and shoves on the frame it vanishes
        let vn = app.world().entity(near).get::<Velocity>().unwrap().0;
        assert!(vn.x > 300.0, "the near rock is thrown outward ({vn:?})");
        assert!(vn.length() <= SUNDER_ROCK_CAP + 0.01, "but clamped under the ship's speed — got {}", vn.length());
        assert_eq!(app.world().entity(far).get::<Velocity>().unwrap().0, Vec2::ZERO, "and the far rock is untouched");
        let vs = app.world().entity(ship).get::<Velocity>().unwrap().0;
        assert!(vs.y > 0.0, "you get shoved too — away from it");
        assert!(vs.length() < MAX_SPEED, "and can always fly out of your own mess");
        assert!(app.world().get_entity(ship).is_ok(), "the shove does NO damage; it only moves things");
        assert_eq!(app.world().resource::<Stats>().sunders, 1);
    }

    // A sunder culled off-screen must not reach back in and shove the field from out there.
    #[test]
    fn a_sunder_culled_offscreen_does_not_shove() {
        let mut app = App::new();
        app.add_plugins(MinimalPlugins);
        app.add_event::<SoundFx>();
        app.insert_resource(bevy::time::TimeUpdateStrategy::ManualDuration(std::time::Duration::from_millis(50)));
        app.insert_resource(Arena { half: Vec2::new(700.0, 400.0) });
        app.insert_resource(Stats::default());
        let sunder = app.world_mut().spawn((Sunder, Transform::from_xyz(900.0, 0.0, 0.0))).id(); // well outside
        let rock = app.world_mut().spawn((
            Asteroid { size: 1, verts: vec![Vec2::X * 22.0], rot: 0.0, spin: 0.0, dense: false, hp: 1 },
            Velocity(Vec2::ZERO),
            Transform::from_xyz(880.0, 0.0, 0.0),
        )).id();
        app.add_systems(Update, sunder_update);
        app.update();
        app.world_mut().entity_mut(sunder).despawn();
        app.update();
        assert_eq!(app.world().entity(rock).get::<Velocity>().unwrap().0, Vec2::ZERO, "an off-screen cull is not a shove");
        assert_eq!(app.world().resource::<Stats>().sunders, 0, "and it doesn't count toward the achievement");
    }

    // Both debut on their own wave and neither leaks earlier.
    #[test]
    fn the_new_ngplus_rocks_debut_on_their_own_waves() {
        // a fresh rng per probe keeps the closure immutable (and each probe independent)
        let seen = |lvl: i32, want: RockKind| {
            let mut rng = rand::thread_rng();
            (0..900).any(|_| roll_ngplus_kind(lvl, &mut rng) == want)
        };
        assert!(!seen(10, RockKind::Binary), "the Binary must not appear before wave 11");
        assert!(seen(11, RockKind::Binary), "wave 11 is its teaching wave");
        assert!(!seen(12, RockKind::Sunder), "the Sunder must not appear before wave 13");
        assert!(seen(13, RockKind::Sunder), "wave 13 is its teaching wave");
        // and the older roster keeps running underneath them — a debut must not evict what came before
        assert!(seen(13, RockKind::Hunter) || seen(13, RockKind::Lapse), "the earlier roster still shows up");
    }
