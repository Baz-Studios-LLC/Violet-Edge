...
| **Binary** | ✅ | **NG+ roster, debuts w11.** Two rocks bound together, turning about their shared centre. The rocks are ordinary; **the LINK between them is the hazard** — a sweeping lethal bar you have to *time*, not an object you steer around. First rock in the game that's about **space** rather than about itself. One roll spawns **two rocks** (`spawn_kind_rock` cross-links the pair), so its spawn rate is deliberately the lowest on the roster — at parity it would eat the whole population budget. Only the `lead` half drives the motion and places the follower each frame, which is what keeps the bar rigid instead of letting collisions stretch it. **The bond is fragile by design:** kill either half and the survivor is flung along its tangent at `BINARY_FLING` (165, SPEED-LAW asserted), so breaking one end is a real answer rather than half a job — it converts a timing problem into an ordinary drifting rock. `BINARY_LINK` (96px) is asserted > 60 so the bar can never hide inside the two rocks it joins. |
| **Sunder** | ✅ | **NG+ roster, debuts w13.** Inert until it dies, and then it **SHOVES**: a shock ring that throws every loose rock — and the ship — outward, with **no damage at all**. The danger is entirely what it puts in your path, so *your own kills become positioning decisions*. It's the counter-pressure to a calmer field: chaos generated by the player's actions rather than by more spawns. `SUNDER_PUSH` 620 at the centre falling off to nothing at `SUNDER_PUSH_R` (300); the ship takes `SUNDER_SHIP_MULT` (0.55) of it, asserted under `MAX_SPEED` so you can always fly out of your own mess. Implemented via **`RemovedComponents<Sunder>`** rather than a `Flavor` flag: the rock can be killed by a bullet, the beam, a blast, the warp or a boss, and that catches every path without touching any of them. A sunder *culled off-screen* doesn't get to shove the field from out there. |
...
- **NEW GAME+ (2026-07-30, deliberately SMALL — user: "start small since it's essentially a New
  ...
  **THE SLINGER+ (2026-08-06) — CLOSE HAUL.** Its tractor beam no longer grabs the rock nearest to
  *itself*; it grabs the one nearest to **you**. The rock you're sheltering behind becomes the round
  fired at you. This is a deliberate, direct answer to **cover** — mob rounds are stopped by rocks
  (see the NO PRIVILEGES law), so hugging one is otherwise a reliable way to sit out a barrage — and it
  drags the fight to wherever you are rather than letting you pick the range. One line of change,
  because the Slinger's whole identity is already *where its ammunition comes from*.
  ℹ️ **It already roams the full arena** and always has: it mirrors your x while sining the full
  height, in its own code rather than via `boss_roam_target`. (A grep for that helper suggests it
  doesn't roam — it does.)
